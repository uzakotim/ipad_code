...
def main():
	...
	rows = []	
	# reading 
	with open("fileEU.csv",'r',encoding='latin-1') as file:
	...
	total_spending = round(sum([x for x in numbers if x<0]),2)
	total_income = sum([x for x in numbers if x>0])

	total_cash_flow = round(total_income+total_spending,2)
